...
export interface DropletsOptions {
  ...
  tintStrength?: number;
  /**
   * Capture and refract the wrapped content. Must be off when an ancestor is
   * already a capture host (e.g. Glass): a nested `layoutsubtree` canvas never
   * receives a paint, so both effects go blank. Off renders the children as
   * normal DOM and draws standalone glass droplets over them.
   */
...
export function createDroplets(
  ...
  /* Window, not the wrapper: as a non-capturing overlay this pane is
   * pointer-events: none so the page underneath stays usable, which means it
   * never receives moves itself. Coordinates stay canvas-relative, and moves
   * outside the canvas are discarded below. */
  const listenTarget: Window | HTMLElement = window;
...
export function Droplets({
  ...
  const native = supported && !failed && (initialOptions.captureContent ?? true);
